    ...
    <div className="py-20 parent bg-neutral">
      <motion.div className="mb-12" initial="hidden" animate={viewDiv && "visible"} variants={headingAnimation}>
        ...
        <h2 className="py-5 text-center text-accent">
          Berkembang Bersama Digital: Desain Inovatif, Fungsi Superior, Solusi Terpercaya untuk Performa Optimal – Layanan Pembuatan Website dan Repair Laptop/Komputer, Kembalikan Keunggulan Teknologimu!
        </h2>
      </motion.div>

      <motion.div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3" ref={ref} initial="hidden" animate={viewDiv && "visible"} variants={sectionBodyAnimation}>
        {Services?.map((service) => (
          <div key={service.id} className={`${service.id % 2 === 0 ? "bg-base-100 impactfull-card shadow-lg" : "bg-base-100 impactfull-card shadow-lg"} rounded-lg p-6 duration-300`}>
